...
if (Nbs>1)
    theta                   = (0:Nbs-2)'*pi/3;
    Cell.Position(2:end,1:2)	= 2*Rcell*[cos(theta) sin(theta)]; %% Why sqrt(3)??
end
...
if (Nbs>7)
    ...
    y     = 2*sqrt(3)*Rcell*sin(theta);
    theta = 0:pi/3:5/3*pi;
%     [x; 2*sqrt(3)*Rcell*cos(theta)];



     x     = reshape([x; 4*Rcell*cos(theta)],numel([x; 2*sqrt(3)*Rcell*cos(theta)]),1);
     y     = reshape([y; 4*Rcell*sin(theta)],numel([y; 2*sqrt(3)*Rcell*sin(theta)]),1);
...
%     figure(4298);%set(gcf,'DockControls','on','WindowStyle','docked','NumberTitle','off','Name','Multi-cell Scenario');
    ...
    for Icell = 1 : Nbs
      ...
      x1 = 2/sqrt(3)*Rcell*cos(-pi/6:pi/3:2*pi) + x0;

      y1 = 2/sqrt(3)*Rcell*sin(-pi/6:pi/3:2*pi) + y0;
